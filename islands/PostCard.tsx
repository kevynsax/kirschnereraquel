...
const createdAtLabel = (createdAt: Date): string => {
    const yearsSinceCreation = new Date().getFullYear() - createdAt.getFullYear();
    ...
    const minutesSinceCreation = new Date().getMinutes() - createdAt.getMinutes();
    ...
    const secondsSinceCreation = new Date().getSeconds() - createdAt.getSeconds();
...
export function PostCard(props: PostCardProps) {
    ...
    effect(() => {
        const interval = setInterval(() => {
            timeSinceCreation.value = createdAtLabel(new Date(props.post.createdAt));
        }, 1000);
